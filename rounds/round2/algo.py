...
def compute_midpoint(order_depth: OrderDepth) -> float:
    if not order_depth.buy_orders or not order_depth.sell_orders:
        return None  # Incomplete book
...
class Trader:
        ...
        if net_position > 0:
            ...
            for price in sorted(order_depth.buy_orders.keys(), reverse=True):  # Highest bid first
                if price >= fair_value:
                    ...
                    if actual_amount > 0:
                        orders.append(
                            Order(product, int(price), -actual_amount))
    ...
    def kelp_orders(
        ...
        if vwap:
            ...
            if total_volume > 0:
                fair_value = (
                    sum(x["vwap"] * x["vol"]
                        for x in self.kelp_vwap) / total_volume
                )
            else:
                fair_value = midpoint
        else:
        ...
        if best_valid_ask in order_depth.sell_orders:
            ...
            if best_valid_ask <= fair_value - take_width and (
                ...
                if quantity > 0:
                    orders.append(
                        Order(Product.KELP, int(best_valid_ask), quantity))
        ...
        if best_valid_bid in order_depth.buy_orders:
            ...
            if best_valid_bid >= fair_value + take_width and (
                ...
                if quantity > 0:
                    orders.append(
                        Order(Product.KELP, int(best_valid_bid), -quantity))
        ...
        if buy_quantity > 0:
            orders.append(Order(Product.KELP, int(
                passive_bid + 1), buy_quantity))
    ...
    def squid_orders(
        ...
        # Factor scaling take_width with volatility
        volatility_take_factor: float = 60.0,
        base_passive_spread: int = 1,  # Minimum passive spread in ticks
        # Factor scaling passive spread with volatility
        volatility_spread_factor: float = 60.0,
        # Factor scaling passive size with volatility (inverse)
        volatility_size_factor: float = 60.0,
        ...
        if best_ask <= fair_value - take_width:
            ...
            if ask_volume > 0:
                ...
                if quantity_to_buy > 0:
                    orders.append(
                        Order(Product.SQUID_INK, int(best_ask), quantity_to_buy))
        ...
        if best_bid >= fair_value + take_width:
            ...
            if bid_volume > 0:
                ...
                if quantity_to_sell > 0:
                    orders.append(
                        Order(Product.SQUID_INK, int(best_bid), -quantity_to_sell))
        ...
        remaining_sell_capacity = position_limit + \
            (position - sell_order_volume)
        ...
        if buy_volume_passive > 0:
            orders.append(
                Order(Product.SQUID_INK, buy_price, buy_volume_passive))

        # Sell order
        if sell_volume_passive > 0:
            orders.append(
                Order(Product.SQUID_INK, sell_price, -sell_volume_passive))
    ...
    def resin_orders(
        ...
        orders = []  # Initialize orders list
        buy_order_volume = 0  # To keep track of the buy and sell volumes
        # Needed to flatten our positions (see flatten() function)
        sell_order_volume = 0
        ...
        if len(order_depth.sell_orders) != 0:
            ...
            if best_ask <= fair_value - take_width:
                ...
                if quantity > 0:
                    orders.append(Order(Product.RAINFOREST_RESIN,
                                  int(best_ask), quantity))
                    buy_order_volume += quantity
        ...
        best_above_ask_fallback = (
            min(sell_candidates) if sell_candidates else fair_value + 2
        )
        buy_candidates = [
            price for price in order_depth.buy_orders.keys() if price < fair_value
        ]
        best_below_bid_fallback = (
            max(buy_candidates) if buy_candidates else fair_value - 2
        )
        ...
        if buy_quantity > 0:
            orders.append(
                Order(
                    Product.RAINFOREST_RESIN, int(
                        best_below_bid_fallback + 2), buy_quantity
                )
            )  # Buy order
        ...
        if sell_quantity > 0:
            orders.append(
                Order(
                    Product.RAINFOREST_RESIN, int(
                        best_above_ask_fallback - 2), -sell_quantity
                )
            )  # Sell order
    ...
    def basket_orders(self, order_depth, position_b1, position_b2, corr_window=100):
        ...
        spread = mid_price['PICNIC_BASKET1'] - (
            3 * mid_price['JAMS'] + 6 *
                mid_price['CROISSANTS'] + mid_price['DJEMBES']
        )

        # Set a threshold for how big the mispricing must be before trading
        # This 0.5 is also from the 2023 code, haven't tried other values yet
        trade_at = self.basket1_std * 0.5
        ...
        if spread > trade_at:
            ...
            if vol_b1 > 0:
                orders_b1.append(
                    Order('PICNIC_BASKET1', worst_buy['PICNIC_BASKET1'], -vol_b1))
        ...
        # BUY signal
            ...
            if vol_b1 > 0:
                orders_b1.append(
                    Order('PICNIC_BASKET1', worst_sell['PICNIC_BASKET1'], vol_b1))
    ...
    def basket_orders1(self, order_depth, position_b1, position_b2, corr_window=100):
        # This version uses basket1 synthetic signal to trade both baskets
        # It also uses correlation to determine the direction of B2
        # As long as the correlation is high, we trade B2 in the same direction as B1
        # Otherwise we trade B2 in the opposite direction
        ...
        # Set a threshold for how big the mispricing must be before trading
        # * 0.5 # This 0.5 is also from the 2023 code, haven't tried other values yet
        trade_at = self.basket1_std * 0.7

        # Determine trading direction for B2 based on correlation
        same_direction = correlation > 0.8  # started with 0.7, just trying random values
    ...
    def mean_reversion_signal(self, product: str, order_depth, window: int = 100, z_entry: float = -2.0, z_exit: float = 4):
        if not order_depth.buy_orders or not order_depth.sell_orders:
            return "HOLD", 0
        ...
        # Update midprice history
        self.midprice_history[product].append(mid)
        if len(self.midprice_history[product]) > window:
            self.midprice_history[product].pop(0)

        # Only signal if we have enough data
        if len(self.midprice_history[product]) < window:
            return "HOLD", 0

        history = self.midprice_history[product]
    ...
    def run(self, state: TradingState):
        ...
        if Product.PICNIC_BASKET1 in state.order_depths:
            ...
            orders_b1, orders_b2 = self.basket_orders1(
